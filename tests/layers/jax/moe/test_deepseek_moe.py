...
class TestMoE2DTP(unittest.TestCase):
    """Tests for MoE with 2D Tensor Parallelism configuration."""

    def setUp(self):
        """Set up a 2D mesh for 2D TP testing."""
        devices = jax.devices()
        self.device_count = len(devices)
        if self.device_count < 4:
            self.skipTest("This test requires at least 4 simulated devices.")

        # Create a 2D mesh with (data, model) axes for 2D TP
        mesh_shape = (2, self.device_count // 2)
        device_mesh_array = np.array(devices).reshape(mesh_shape)
    ...
    def test_2d_tp_moe_with_model_sharding(self):
        """Test MoE with 2D TP sharding using MODEL axis."""
        with self.mesh:
            router = Router(
                dtype=self.dtype,
                hidden_size=self.D,
            ...
            # Use 2D TP style sharding - shard on 'model' axis
            moe = MoE(
                dtype=self.dtype,
                num_local_experts=self.E,
                hidden_size=self.D,
                ...
                mesh=self.mesh,
                activation_ffw_td=PartitionSpec('data', 'model'),
                activation_ffw_ted=PartitionSpec('data', None, 'model'),
                # 2D TP style: shard experts and weights on model axis
                edf_sharding=PartitionSpec(None, 'model', None),
                efd_sharding=PartitionSpec(None, None, 'model'),
                apply_expert_weight_before_computation=False,
                moe_backend=MoEBackend.DENSE_MAT,
                num_experts_per_tok=self.K,
            )
    ...
    def test_2d_tp_sharding_axes_available(self):
        """Verify 2D TP sharding configuration is accessible."""
        from tpu_inference.layers.common.sharding import ShardingAxisNameBase

        # Verify MODEL_1 and MODEL_2 are available for 2D TP
